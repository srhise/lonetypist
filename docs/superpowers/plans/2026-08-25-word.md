...
- **The last grid row is the status line.** Text viewport is 23 rows in `80x25`, 48 rows in `80x50` (row 0 is a blank top margin so the menu bar never covers the cursor).
...
pub enum Overlay {
    None,
    Message { title: String, body: String, danger: bool },
